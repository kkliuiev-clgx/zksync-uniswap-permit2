...
const UINT160_MAX: BigNumber = ethers.BigNumber.from(2)
  .pow(ethers.BigNumber.from(160))
  .sub(1);
const DECIMAL_MULT: BigNumber = ethers.BigNumber.from(10).pow(
  ethers.BigNumber.from(18),
);

const PERMIT_DEPLOYER_PK: string =
  "0x7f2f89394abc81f86bf66c81852b9f203a329e4b89af8cd7177807f0178a5e12";
...
describe("Permit2Lib", function () {
  let PERMIT_TYPEHASH: string = ethers.utils.keccak256(
    ethers.utils.toUtf8Bytes(
  ...
  before(async function () {
    ...
    token = <MockERC20>(
      await deployContract("MockERC20", ["Test", "MOCK", DECIMALS])
    );

    nonPermitToken = <MockNonPermitERC20>(
      await deployContract("MockNonPermitERC20", ["Test", "MOCK", DECIMALS])
    );
    lessDSToken = <MockPermitWithSmallDS>(
      await deployContract("MockPermitWithSmallDS", ["Test", "MOCK", DECIMALS])
    );
    largeDSToken = <MockPermitWithLargerDS>(
      await deployContract("MockPermitWithLargerDS", ["Test", "MOCK", DECIMALS])
    );
    largerNonStandardDSToken = <MockNonPermitNonERC20WithDS>(
      await deployContract("MockNonPermitNonERC20WithDS")
    );
  ...
  beforeEach(async function () {
    ...
    let allowanceBefore = await permit2
      .connect(wallet)
      .allowance(PK_OWNER.address, token.address, CAFE.address);
    let permitSingle: PermitSingle = buildPermitSingle(
      token.address,
      DECIMAL_MULT,
      UINT48_MAX,
      allowanceBefore.nonce,
      CAFE.address,
      blockTimestamp,
    );
    let sign = getPermitSignatureSeparated(
      permitSingle,
      PK,
    ...
    let allowanceMiddle = await permit2
      .connect(wallet)
      .allowance(PK_OWNER.address, nonPermitToken.address, CAFE.address);

    permitSingle = buildPermitSingle(
      nonPermitToken.address,
      DECIMAL_MULT,
      UINT48_MAX,
      allowanceMiddle.nonce,
      CAFE.address,
      blockTimestamp,
    );

    sign = await getPermitSignatureSeparated(
      permitSingle,
      PK,
      PERMIT2_DOMAIN_SEPARATOR,
    );
      ...
      let hash: string = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(
          ...
          [
            ...
            BOB.address,
            DECIMAL_MULT,
            await token.connect(PK_OWNER).nonces(PK_OWNER.address),
            blockTimestamp,
          ],
        ),
      );
  ...
  describe("Test Permit2 Invalid Amount", function () {
    it("should revert with invalid amount", async function () {
      ...
      const sign = getPermitSignatureSeparated(
        permit,
        PK,
        PERMIT2_DOMAIN_SEPARATOR,
      );
  ...
  describe("Test Standard TransferFrom", function () {
    it("TransferFrom should not revert", async function () {
      ...
      await (
        await token
          .connect(wallet)
          .transferFrom(wallet.address, randomAddress, DECIMAL_MULT)
      ).wait();
    });
  });
  ...
  describe("Test Open-Zeppelin SafeTransferFrom", function () {
    it("SafeTransferFrom should not revert", async function () {
      await (
        await token
          .connect(wallet)
          .approve(safeERC20.address, ethers.constants.MaxUint256)
      ).wait();
      await expect(
        safeERC20
          .connect(wallet)
  ...
  describe("Test TransferFrom2", function () {
    it("TransferFrom2 should not revert", async function () {
      await (
        await token
          .connect(wallet)
          .approve(permit2Lib.address, ethers.constants.MaxUint256)
      ).wait();
      await expect(
        permit2Lib
          .connect(wallet)
  ...
  describe("Test Permit2 Smaller DS", function () {
    it("Permit2 with Smaller DS should not revert", async function () {
      let result = await permit2
        .connect(wallet)
        .allowance(PK_OWNER.address, lessDSToken.address, CAFE.address);
  ...
  describe("Test Permit2 Larger DS", function () {
    it("Permit2 with Larger DS should not revert", async function () {
      let result = await permit2
        .connect(wallet)
        .allowance(PK_OWNER.address, largeDSToken.address, CAFE.address);

      let permit: PermitSingle = buildPermitSingle(
        largeDSToken.address,
        DECIMAL_MULT,
        UINT48_MAX,
        result.nonce,
        CAFE.address,
      ...
      let permitHash: string = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(
          ...
          [
            ...
            BOB.address,
            DECIMAL_MULT,
            await lessDSToken.connect(PK_OWNER).nonces(PK_OWNER.address),
            blockTimestamp,
          ],
        ),
      );
  ...
  describe("Test TransferFrom2 InvalidAmount", function () {
    it("TransferFrom2 with invalid amount should revert", async function () {
      await (
        await token
          .connect(PK_OWNER)
          .approve(CAFE.address, ethers.constants.MaxUint256)
      ).wait();
      await expect(
        (
          await permit2Lib
  ...
  describe("Test Permit2 DSLessToken", function () {
    it("should be true", async function () {
      await expect(
        await permit2Lib.connect(wallet).testPermit2Code(lessDSToken.address),
      ).to.be.true;
    });
  });
  ...
  describe("Test Permit2 DS More Token", function () {
    it("should be false", async function () {
      await expect(
        await permit2Lib
          .connect(wallet)
          .testPermit2Code(largerNonStandardDSToken.address),
      ).to.be.false;
    });
  });
  ...
  describe("Test Permit2 DS More 32 Token", function () {
    it("should be false", async function () {
      await expect(
        await permit2Lib.connect(wallet).testPermit2Code(largeDSToken.address),
      ).to.be.false;
    });
  });
});
